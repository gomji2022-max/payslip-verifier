...
- "DO worked" flag -> entire shift treated as O/TIME 200% (ASSUMPTION, needs verification)
- "48h notice change" flag -> overrides tier with a flat 150% or 200% for the whole shift
...
if(shift.dayOffWorked){
   ...
   if(shift.scheduleChange){
      ...
      const nightEnd = overnight && outTod < 8*60;
...
async function ensureOcrLibs(){
   await loadScriptOnce('https://cdn.jsdelivr.net/npm/tesseract.js@5.0.5/dist/tesseract.min.js');
...
async function fileToCanvas(file){
   ...
   const targetW = Math.max(img.width, 1600);
...
const OCR_CODE_MAP = [
   { key:'basic', patterns:[/\bBASIC\b/i] },
   { key:'p225', patterns:[/22\.?5\s*%\s*SHIFT/i] },
   { key:'p175', patterns:[/17\.?5\s*%\s*SHIFT/i] },
   { key:'p15',  patterns:[/\b15\s*%\s*SHIFT/i] },
   { key:'p100', patterns:[/100\s*%\s*SHIFT/i] },
   { key:'p50',  patterns:[/\b50\s*%\s*SHIFT\b(?!.*NIL)/i] },
   { key:'ot200',patterns:[/O\s?\/?\s?TIME\s*200\s*%?/i] },
   { key:'transport', patterns:[/TRANSPORT/i] },
   { key:'tea', patterns:[/\bTEA\b/i] }
   ];
const OCR_SKIP_LINE = /PAYSLIP|QANTAS|Earnings|Detailed Leave|Leave Type|Pay Summary|Total (Gross|Net)|Taxable Gross|Less (Tax|Deductions)|Banking|Institution|Branch|Tax Scale|Fund\s*\/|Account Name|Super|Date of Release|Customer Service|Airport|Code\s+Date|Periods Payments|Current Rate|Rate\s*&\s*PD|Untaxed Items|Messages|People Services/i;

function parsePayslipText(text){
   const lines = text.split('\n').map(l=>l.trim()).filter(Boolean);
   const numRe = /\d[\d,]*\.\d{1,4}|\.\d{1,4}/g;
   const result = {};
   const usedIdx = new Set();
   lines.forEach((line, idx)=>{
      const match = OCR_CODE_MAP.find(c => c.patterns.some(p => p.test(line)));
      if(!match) return;
      const nums = (line.match(numRe) || []).map(n => parseFloat(n.replace(/,/g,'')));
      if(nums.length >= 2){
         const hrs = nums[nums.length - 2];
         result[match.key] = (result[match.key] || 0) + hrs;
         usedIdx.add(idx);
      }
   });
   const otherLines = [];
   lines.forEach((line, idx)=>{
      ...
      if(OCR_SKIP_LINE.test(line)) return;
      if(!/^[A-Za-z0-9]/.test(line)) return;
      const label = line.split(/\s{2,}/)[0].slice(0, 30);
      if(!/[A-Za-z]/.test(label)) return;
      const nums = (line.match(numRe) || []).map(n => parseFloat(n.replace(/,/g,'')));
      if(nums.length === 0) return;
      const amount = nums[nums.length - 1];
      if(amount <= 0 || amount > 20000) return;
      otherLines.push({ label, amount });
   });
   return { result, otherLines };
}

document.getElementById('fPayslipScan').addEventListener('change', async (e)=>{
   const file = e.target.files[0];
   if(!file) return;
   const statusEl = document.getElementById('scanStatus');
   statusEl.textContent = 'Loading scanning engine (first time only)...';
   try{
      await ensureOcrLibs();
      statusEl.textContent = 'Reading the file...';
      const canvas = await fileToCanvas(file);
      statusEl.textContent = 'Scanning text — this can take 15–40 seconds...';
      const { data: { text } } = await Tesseract.recognize(canvas, 'eng');
      const parsed = parsePayslipText(text);
      let filledCount = 0;
      Object.keys(parsed.result).forEach(k=>{
         const el = document.getElementById('ps_' + k);
         if(el){ el.value = Math.round(parsed.result[k] * 100) / 100; filledCount++; }
      });
      parsed.otherLines.slice(0, 10).forEach(o=>{
         document.getElementById('btnAddOtherLine').click();
         const rows = document.querySelectorAll('#otherLines .row2');
         const last = rows[rows.length - 1];
         last.querySelector('.other-label').value = o.label;
         last.querySelector('.other-amount').value = o.amount;
      });
      if(filledCount === 0 && parsed.otherLines.length === 0){
         statusEl.innerHTML = '⚠️ Could not confidently read any line items from this file. Please enter the numbers manually below.';
      } else {
         statusEl.innerHTML = `✅ Filled in ${filledCount} line item(s) and ${Math.min(parsed.otherLines.length,10)} other item(s) from the scan. ` +
            `<b>OCR makes mistakes — please check every number against your real payslip before comparing.</b>`;
      }
      document.getElementById('payslipFields').scrollIntoView({ behavior:'smooth' });
   }catch(err){
      statusEl.textContent = 'Scan failed (' + err.message + '). Please enter the numbers manually below.';
   }
});
